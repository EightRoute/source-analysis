...
class LocalCache<K, V> extends AbstractMap<K, V> implements ConcurrentMap<K, V> {
    ...
    /*
     * 类似1.7的ConcurrentHashMap采用多Segment减少并发的阻塞
     *
     * 使用了LRU算法,需要设置最大容量，基于accessQueue
     */
    ...
    /**
     * 每次执行postReadCleanup操作时readCount都增1，
     * 当其达到64时（DRAIN_THRESHOLD为0x3F，即0011 1111），引发cleanUp操作。
     * DRAIN_THRESHOLD必须为(2^n)-1作为他的值
     */
    ...
    /**
     * 定位segment，segmentShift和segmentMask用来打散entry，让缓存内容尽量均匀分布在每个segment下
     */
    ...
    enum Strength {
        ...
        SOFT {
            ...
            /**
             * 判断两个对象是否相等时会直接返回false
             * hash值调用System.identityHashCode直接返回系统默认的值
             */
            @Override
            Equivalence<Object> defaultEquivalence() {
        ...
        abstract <K, V> ValueReference<K, V> referenceValue(Segment<K, V> segment, ReferenceEntry<K, V> entry,
                                                            V value, int weight);
    ...
    /**
     * 对于不需要维护WriteQueue和AccessQueue的配置（即没有expire time或size limit的evict策略）来说，
     * 我们可以使用DISCARDING_QUEUE以节省内存
     */
    ...
    static class Segment<K, V> extends ReentrantLock {
        ...
        /**
         * 阈值，大于这个值时就需要扩容，一般为capacity的0.75
         */
        int threshold;
        ...
        /**
         * segment最大weight的值. UNSET_INT if there is no maximum.
         */
        final long maxSegmentWeight;

        /**
         * key已经被回收了的entry的引用队列
         */
        final @Nullable ReferenceQueue<K> keyReferenceQueue;

        /**
         *  value已经被回收了的entry的引用队列
         */
        final @Nullable ReferenceQueue<V> valueReferenceQueue;

        /**
         * recencyQueue启用条件和accessQueue一样。
         * recencyQueue是为accessQueue服务的,在无锁的情况使用recencyQueue
         * recencyQueue是ConcurrentLinkedQueue,无需加锁，可提高效率
         */
        ...
        /**
         * 写操作的引用队列,开启expireAfterWrite才有
         */
        @GuardedBy("this")
        final Queue<ReferenceEntry<K, V>> writeQueue;

        /**
         * 操作的引用队列,开启expireAfterAccess才有
         */
        @GuardedBy("this")
        ...
        Segment(LocalCache<K, V> map, int initialCapacity, long maxSegmentWeight, StatsCounter statsCounter) {
            ...
            //只有recencyQueue是ConcurrentLinkedQueue
        ...
        /**
         * 给entry设置一个新的值. 将新创建的entry加入到access queue的尾部.
         */
        @GuardedBy("this")
        ...
        V lockedGetOrLoad(K key, int hash, CacheLoader<? super K, V> loader) throws ExecutionException {
            ...
            /*为防止多线程并发下重复加载，需要先锁定，
             获得加载资格的线程（获得锁的线程）创建
             一个LoadingValueRefrerence并放入map中，其他线程等待结果返回。*/
            ...
            try {
                ...
                if (createNewEntry) {
                    ...
                    //为防止多线程并发下重复加载，需要先锁定，获得加载资格的线程（获得锁的线程）
                    // 创建一个LoadingValueRefrerence并放入map中，其他线程等待结果返回。
                    if (e == null) {
        ...
        /**将回收的缓存（包含了 key，value）以及回收原因包装成之前定义的事件接口
        加入removalNotificationQueue队列*/
        @GuardedBy("this")
        ...
        /**
         * 当entry是invalid, partially-collected,
         * loading, or expired.返回null
         */
        V getLiveValue(ReferenceEntry<K, V> entry, long now) {
            if (entry.getKey() == null) {
                tryDrainReferenceQueues();
    ...
    /**
     *  Write队列
     *  所有元素在map中就在queue中，所有元素不在queue中的就不在map中
     *  好处
     *  (1) 我们可以替换中间的元素作为copywritry的一部分，
     *  (2) 对包含的方法进行了高度的优化
     */
    ...
    /**
     * Serializes the configuration of a LocalCache,
     */
